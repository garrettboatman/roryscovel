...
const Header = ({ siteTitle }) => {
  ...
  <header style={{zIndex: 100, position: 'relative'}} className={`content-container py-3 ${menuOpen ? `is-open` : ``} `}>
    <div className="flex mx-auto max-w-[1000px] relative justify-center md:justify-start items-center">
      ...
      <div className={`absolute transition-all position-absolute translate-x-[200px] opacity-0 overflow-hidden w-0 left-[-20px] md:left-[-30px] top-[40px] rounded-lg z-50 bg-[#fff6e5] shadow-lg ${menuOpen ? `w-auto !translate-x-[0px] opacity-100` : ``}`}>
